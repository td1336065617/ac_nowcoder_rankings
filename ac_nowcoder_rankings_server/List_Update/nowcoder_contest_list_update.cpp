    ...
    int ac_nowcoder_rankings_server::nowcoder_contest_list_update(long long int contestId) {
        ...
        for (auto &x: Memorize_the_assessment_records[contestId]) {
            ...
            if (x.second.get_status_message() == "AC") {
                if (nowcoder_contest_list[contestId].find(x.second.get_user_id()) == nowcoder_contest_list[contestId].
                    end()) {
                } else {
                    nowcoder_contest_list_mtx.lock();
                    if (nowcoder_contest_list[contestId][x.second.get_user_id()].Title_status_map[x.second.get_index()].
                        status) {
                        continue;
                        nowcoder_contest_list_mtx.unlock();
                    }
                    ac_nowcoder_Ranking_data ac_nowcoder_ranking_data1 = nowcoder_contest_list[contestId][x.second.
                        get_user_id()];

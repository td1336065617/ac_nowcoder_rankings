...
namespace ac_nowcoder_rankings_server {
    ...
    // @param page: 获取页码
    // @param max_pageSize: 单页获取数据量
    // @return: 返回一个包含评估数据的vector
    vector<Evaluation_Data_Template> ac_nowcoder_rankings_server::
    Get_Evaluation_Data_Single_request(long long int contestId, string cookie, int page, int max_pageSize) {
        try {
            vector<Evaluation_Data_Template> Evaluation_Data_Template_list; // 用于存储评估数据的vector
            ...
            string url = "https://ac.nowcoder.com/acm-heavy/acm/contest/status-list?token=&id="
                         + std::to_string(contestId) + "&page=" + std::to_string(page) + "&pageSize=" + std::to_string(max_pageSize) +
                         "&searchUserName=&_="

...
namespace ac_nowcoder_rankings_server {
    int ac_nowcoder_rankings_server::Task_distribution_center(HTTP_request_data httpRequestData,
        ...
        if (httpRequestData.url=="/get_nowcoder_contest_list.jk") {
            ...
            httpRequestData.http_data_value["idx_begin"]=1;
            httpRequestData.http_data_value["size"]=50;
            long long int contestId=stoll( httpRequestData.http_data_value["contest"]);
            int idx_begin=stoi(httpRequestData.http_data_value["idx_begin"]);
            int size=stoi(httpRequestData.http_data_value["size"]);
            if (idx_begin<=0) {
                idx_begin=1;
            }
            if (size<=0) {
            ...
            httpResponseData.http_response_data_value=get_nowcoder_contest_list(contestId,idx_begin,size);
        }
        // 2. 添加新的牛客竞赛监听器请求
        ...
        }  else if (httpRequestData.url=="/get_nowcoder_user_info.jk") {
            httpRequestData.http_data_value["contest"]="106871";
            string  get_nowcoder_user_info_str=get_nowcoder_user_info(std::stoll(httpRequestData.http_data_value["contest"]));
            httpResponseData.http_response_data_value=get_nowcoder_user_info_str;
        }
